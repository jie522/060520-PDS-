...
說明：
  ...
  - 機型 (ji_xing)  : 優先讀 PDM 資料卡「機型」變數（從 PDM SQL DB 直查，跟品名同一批查詢）；
                      卡片沒填值時，才 fallback 用資料夾路徑推導（如 FTD\\FTD430B\\ -> FTD430B）。
                      2026-07 發現 TCW 系列圖檔直接放在系列資料夾下（無機型子資料夾），
                      純路徑推導會讓整個系列都顯示成資料夾名稱「TCW」，改用卡片變數後才正確。
...
def connect_vault():
    ...
    2026-07 新增：多台電腦透過網路共用資料夾跑同一份 dist_embed 時，pywin32 的
    gen_py 快取（型別庫 wrapper 產生後的暫存模組）若在某台電腦上損毀或跟該機器
    實際登錄的 COM 型別庫版本對不上，會炸出類似
    `AttributeError: module 'win32com.gen_py.<LIBID>...' has no attribute 'CLSIDToPackageMap'`
    （實測發現這個錯誤有好幾種變體，缺的屬性可能是 CLSIDToPackageMap、CLSIDToClassMap、
    VTablesToPackageMap 等，共同特徵是訊息裡一定會有 `win32com.gen_py`，所以判斷式
    抓這個關鍵字，不要只比對單一屬性名稱）。且不只發生在這裡的 EnsureDispatch，
    後面任何 CastTo 呼叫也可能觸發同一種錯誤（因為整個型別庫的 wrapper package 都
    沒正確產生）。單純 except 改用晚期繫結無法解決，因為 CastTo 一定要早期繫結才能用
    （見 docs/pdm-api-cookbook.md）。
    修法：偵測到這個特徵錯誤時，清掉 gen_py 快取資料夾讓 pywin32 重新產生一份
    乾淨、跟這台機器登錄版本吻合的 wrapper，再重試一次連線。
    """
    def _ensure_dispatch():
        from win32com.client import gencache
        return gencache.EnsureDispatch('ConisioLib.EdmVault')

    def _is_genpy_cache_error(e):
        return isinstance(e, AttributeError) and 'win32com.gen_py' in str(e)

    try:
        v = _ensure_dispatch()
    except AttributeError as e:
        if not _is_genpy_cache_error(e):
            raise
        try:
...
def load_vars_from_sql(vault_name=VAULT_NAME, log=None):
    """
    直接透過 ADO 連線 PDM SQL Server，讀取所有 SLDDRW 的 TARGET_VARS（品名/發圖日期/機型）。
    回傳 dict: {filename_lower: {變數名: str, ...}}（key 依 TARGET_VARS 而定）
    """
    def _log(msg):
        print(msg)
        if log:
...
def load_vars_via_com(vault, results, log=None):
    ...
    讀取 TARGET_VARS（品名/發圖日期/機型）。這些方法會從 PDM Archive Server 取值，不需本機快取。
    回傳 dict: {filename_lower: {變數名: str, ...}}（key 依 TARGET_VARS 而定）
    """
    def _log(msg):
        print(msg)
...
def rebuild(update_only=False, deploy=False):
    ...
    for idx, (folder_path, fname) in enumerate(results, 1):
        ...
        try:
            ...
            # 機型：優先用 PDM 資料卡的「機型」變數（工程師實際填的值），
            # 卡片沒填時才 fallback 用資料夾路徑推導（見 derive_ji_xing 說明）
            ji_xing = sql_row.get('機型', '').strip() or derive_ji_xing(folder_path)
            xing_hao = ji_xing
...
def _deploy(src):
    ...
    dist_dir   = os.path.join(script_dir, 'dist_embed', 'PDS\u7cfb\u7d71')
