...
def try_get_var(ev, var_name, version):
    """嘗試讀取 PDM 自訂變數（未快取時通常為空）"""
    try:
...
def rebuild(update_only=False, deploy=False):
    ...
    for idx, (folder, file_obj) in enumerate(results, 1):
        ...
        try:
            ...
            # 嘗試讀取 PDM 變數（通常需要檔案已本機快取才有值）
            ev = file_obj.GetEnumeratorVariable()
            ver = file_obj.CurrentVersion
            tu_mian_pin_ming = try_get_var(ev, '品名', ver)
            modified_at      = try_get_var(ev, '發圖日期', ver)
